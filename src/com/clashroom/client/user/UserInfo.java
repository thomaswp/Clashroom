...
public class UserInfo extends Composite {
	...
	public void setUser(UserEntity user) {
		...
		for (Skill skill : dragonClass.getSkills()) {
			addSkill(skill, user, dragonClass);
		}
	}
	
	private void addSkill(final Skill skill, UserEntity user, DragonClass dragonClass) {
		...
		if (!learned) {
			...
			if (reqLevel <= dragon.getLevel() && user.getSkillPoints() >= skill.getSkillPointCost()) {
				final Anchor anchorSkillPoint = new Anchor(cost);
				anchorSkillPoint.addClickHandler(new ClickHandler() {
					@Override
					public void onClick(ClickEvent event) {
						anchorSkillPoint.setEnabled(false);
						Debug.write(userInfoService);
